...
import logging
lg = logging.getLogger("pod2markdown")
...
`--oformat` is unfinished. It mainly just does 'mediawiki', but 'html'
...
Start support for alternate `--oformat` settings.
...
def escapeXmlContent(s):  # From my utils
...
if __name__ == "__main__":
    def processOptions() -> argparse.Namespace:
        ...
        if (args0.verbose): logging.basicConfig(level=logging.INFO - args0.verbose)
        return(args0)
...
"""
    ...
    if (len(args.files) == 0):
        ...
        for path0 in args.files:
            fh0 = codecs.open(path0, "rw", encoding=args.iencoding)
            doOneFile(path0, fh0)
